...
const counterReducers = (state = initialState, action) => {
  if (action.type === information.INCREMENT) {
    return {
      ...state,
      ...(state.score[action.payload.index].value =
        state.score[action.payload.index].value + action.payload.value),
    };
  } else if (action.type === information.DECREMENT) {
    if (state.score[action.payload.index].value - action.payload.value < 0) {
      return { ...state, ...(state.score[action.payload.index].value = 0) };
    }
    return {
      ...state,
      ...(state.score[action.payload.index].value =
